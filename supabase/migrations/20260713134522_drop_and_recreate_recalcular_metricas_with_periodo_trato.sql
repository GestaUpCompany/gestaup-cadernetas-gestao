DROP FUNCTION IF EXISTS recalcular_metricas_suplementacao(UUID);
...
BEGIN
  ...
  FOR v_fazenda_idx IN 1 .. array_length(v_fazendas, 1) LOOP
    ...
    LOOP
      ...
      v_data_registro := v_registro.data_reg;

      -- Calcular período de trato (dias desde o último registro do mesmo lote)
      SELECT (v_data_registro - MAX(r.data::date))::integer
      INTO v_periodo_trato
      FROM registros_suplementacao r
      WHERE r.fazenda_id = v_fazenda
        AND r.lote_id = v_lote_id
        AND r.deleted_at IS NULL
        AND r.data::date < v_data_registro;
      ...
      IF v_peso_vivo_medio IS NULL OR v_peso_vivo_medio = 0 OR v_animais_elegiveis IS NULL OR v_animais_elegiveis = 0 THEN
        ...
        SET periodo_trato_dias = v_periodo_trato, updated_at = NOW()
      ...
      SET
        ...
        periodo_trato_dias = v_periodo_trato,
        updated_at = NOW()
      WHERE id = v_registro.id;
