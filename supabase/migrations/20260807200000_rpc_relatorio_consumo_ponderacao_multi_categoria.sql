-- RPC get_dados_relatorio_consumo: ponderação multi-categoria
-- Substitui o LATERAL LIMIT 1 por agregação de todas as categorias ativas do lote.
-- Lotes com categorias faltando peso_entrada, peso_vivo_atual ou quant_atual
-- retornam campo "erro" com a lista de categorias e dados faltantes,
-- e não entram no PDF mas continuam aparecendo no dropdown.
--
-- Nota: a função get_relatorio_consumo (versão interna com p_fazenda_id_param)
-- também foi atualizada com a mesma lógica em migration separada.
...
BEGIN
  ...
  -- Agregar todas as categorias ativas de cada lote
  cats_por_lote AS (
    SELECT
      lcr.lote_id,
  ...
  -- Detectar categorias com dados faltantes
  cats_com_erro AS (
    SELECT
      lote_id,
      jsonb_agg(jsonb_build_object(
  ...
  info_lotes AS (
    SELECT
      cpl.lote_id,
      l.nome AS lote_nome,
      -- Média ponderada de peso de entrada (NULL se houver erro)
      ...
      -- Data de início do plano mais antigo (NULL se houver erro)
      CASE WHEN cce.erros IS NULL THEN to_char(MIN(cpl.data_inicio), 'YYYY-MM-DD') END AS data_inicio_plano,
      -- Dias desde o plano mais antigo (NULL se houver erro)
