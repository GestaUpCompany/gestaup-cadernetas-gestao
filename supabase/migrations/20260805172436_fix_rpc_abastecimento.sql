...
BEGIN
  ...
  -- Agregacao por maquina/veiculo
  SELECT COALESCE(jsonb_agg(jsonb_build_object('label', maquina_veiculo, 'valor', SUM(total_abastecido))), '[]'::jsonb)
  INTO v_por_maquina
  ...
  -- Agregacao por combustivel
  SELECT COALESCE(jsonb_agg(jsonb_build_object('label', combustivel, 'valor', SUM(total_abastecido))), '[]'::jsonb)
  ...
  -- Agregacao por operacao
  SELECT COALESCE(jsonb_agg(jsonb_build_object('label', COALESCE(NULLIF(tipo_operacao, ''), tipo_operacao_outros, 'Nao informado'), 'valor', SUM(total_abastecido))), '[]'::jsonb)
  INTO v_por_operacao
  FROM registros_abastecimento
  WHERE fazenda_id = v_fazenda_id AND deleted_at IS NULL
    AND (p_data_inicio IS NULL OR data::date >= p_data_inicio)
