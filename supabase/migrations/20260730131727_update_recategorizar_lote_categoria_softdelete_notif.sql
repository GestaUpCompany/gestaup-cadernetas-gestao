-- F1.3: Adicionar soft-delete de notificacoes na RPC recategorizar_lote_categoria
-- Quando o produtor recategoriza, as notificacoes de recategorizacao do lote_categoria_id origem
-- sao soft-deletadas (deleted_at = now()), sumindo do sino automaticamente.
...
DECLARE
  ...
  v_novo_peso_meta numeric;
  v_metricas_plano jsonb := NULL;
  v_performance_plano jsonb := NULL;
BEGIN
  -- 1. Carregar origem
  ...
  -- 4. Snapshot da lote_categoria origem
  ...
  -- 5. Snapshot do plano nutricional ativo
  ...
  -- 6. Determinar formulacao_id (bugfix: COALESCE com plano origem)
  IF p_manter_formulacao THEN
    v_novo_formulacao_id := COALESCE(v_origem.formulacao_id, v_plano_origem.formulacao_id);
  ELSE
    v_novo_formulacao_id := p_nova_formulacao_id;
  END IF;

  -- 7. Encerrar plano nutricional ativo da origem
  IF v_tem_plano_origem THEN
    PERFORM public.encerrar_plano_nutricional(p_lote_categoria_origem_id);

    -- 7b. Ler de volta o snapshot de performance
    SELECT 
      jsonb_build_object(
        'duracao_dias', ps.duracao_dias,
        'ganho_peso_total_kg_cab', ps.ganho_peso_total_kg_cab,
        'gmd_realizado', ps.gmd_realizado,
        'gmd_planejado', ps.gmd_planejado,
        'producao_arroba_lote', ps.producao_arroba_lote,
        'mortalidade_percent', ps.mortalidade_percent,
        'motivo_migracao', ps.motivo_migracao,
        'tipo_snapshot', ps.tipo_snapshot,
        'plano_anterior_id', ps.plano_anterior_id,
        'plano_posterior_id', ps.plano_posterior_id
      ),
      ps.metricas_derivadas
    INTO v_performance_plano, v_metricas_plano
    FROM public.planos_nutricionais_snapshots ps
    WHERE ps.plano_nutricional_id = v_plano_origem.id
      AND ps.lote_categoria_id = p_lote_categoria_origem_id
  ...
  IF v_novo_formulacao_id IS NOT NULL THEN
    ...
    IF NOT p_manter_formulacao THEN
      v_novo_peso_meta := COALESCE(v_formulacao.peso_vivo_medio, v_peso_transicao);
    ELSIF v_tem_plano_origem THEN
      v_novo_peso_meta := COALESCE(v_plano_origem.peso_meta_kg, v_formulacao.peso_vivo_medio, v_peso_transicao);
    ELSE
      v_novo_peso_meta := COALESCE(v_formulacao.peso_vivo_medio, v_peso_transicao);
    END IF;
    ...
    IF v_tem_plano_origem THEN
      INSERT INTO public.planos_nutricionais (
        ...
        COALESCE(v_plano_origem.periodo_dias, 0), v_novo_peso_meta, 0, true, CURRENT_DATE,
    ...
    ELSIF NOT p_manter_formulacao THEN
      INSERT INTO public.planos_nutricionais (
        ...
        v_novo_lote_categoria_id, v_fazenda_id, v_novo_nome_plano, v_novo_formulacao_id,
        0, v_novo_peso_meta, 0, true, CURRENT_DATE,
  ...
  INSERT INTO public.lote_categorias_transicoes (
    ...
    jsonb_build_object(
      ...
      'metricas_plano_nutricional', v_metricas_plano,
      'performance_plano_nutricional', v_performance_plano,
      'manter_formulacao', p_manter_formulacao,
      'nova_formulacao_id', p_nova_formulacao_id
    )
  );

  -- 12. Soft-delete notificacoes de recategorizacao do lote_categoria_id origem
  -- Assim, quando o produtor recategoriza, o alerta some do sino automaticamente
  UPDATE public.notificacoes
  SET deleted_at = now()
  WHERE dados_jsonb->>'lote_categoria_id' = p_lote_categoria_origem_id::text
    AND dados_jsonb->>'tipo_alerta' = 'recategorizacao'
...
SELECT 'RPC recategorizar_lote_categoria atualizada com soft-delete de notificacoes' AS status;;
