-- Corrigir função update_dados_lotes removendo campo inexistente
...
BEGIN
  FOR categoria_record IN 
    SELECT lc.id, lc.lote_id, lc.categoria, lc.peso_entrada_kg_cab, lc.gmd, lc.data_pesagem, lc.data_meta_projetada, lc.rc_inicial
    ...
    WHERE lc.peso_entrada_kg_cab IS NOT NULL 
      ...
      AND lc.ativo = true
  LOOP
    -- Converter GMD de text para numeric
    BEGIN
      gmd_value := categoria_record.gmd::NUMERIC;
    ...
    -- STEP 6: Update all in same transaction - apenas campos que existem
