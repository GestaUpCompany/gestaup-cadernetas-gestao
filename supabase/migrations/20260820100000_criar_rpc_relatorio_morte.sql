-- RPC para relatório público de mortalidade.
-- Segue o mesmo padrão da get_dados_relatorio_tratos:
...
--   causas_disponiveis:       [text]                              (slicer)
--   categorias_disponiveis:   [text]                              (slicer)
--   sexos_disponiveis:        [text]                              (slicer)
--   linhas: [{ id, data, lote_id, lote_nome, pasto, sexo, raca, idade,
--             peso_vivo, causa_morte, categoria, categoria_outros,
--             brinco, chip, escore, nutricao_atual, nutricao_anterior,
--             diagnosticos, observacao_identificacao, nome_usuario }]
--   resumo: { total_mortes, media_por_dia, peso_medio,
--             causa_mais_frequente, causa_mais_frequente_count,
--             por_causa: [{ label, valor }],
--             por_categoria: [{ label, valor }],
--             por_sexo: [{ label, valor }],
--             frequencia_diagnosticos: [{ label, valor }] }
...
BEGIN
  ...
  -- Lotes disponíveis (slicer): lotes com pelo menos um registro de morte
  ...
  -- Resumo agregado (calculado a partir das linhas filtradas por data)
  SELECT
    COUNT(*),
  ...
  -- Resumo final com agregações por causa, categoria, sexo e frequência de diagnósticos
  WITH base AS (
    SELECT
      r.causa_morte,
      r.categoria,
  ...
  -- Frequência de diagnósticos: para cada chave do JSONB, conta quantas mortes têm valor='S'
  diag_keys AS (
