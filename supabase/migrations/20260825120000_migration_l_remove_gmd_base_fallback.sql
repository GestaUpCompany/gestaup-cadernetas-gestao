-- Atualizar iniciar_plano_lote para não usar gmd base como fallback
...
BEGIN
  ...
  LOOP
    -- GMD: apenas de formulacao_categorias_gmd, NULL se nao contemplada
...
-- Atualizar encerrar_plano_lote para não usar gmd base como fallback
...
BEGIN
  ...
  LOOP
    ...
    -- GMD planejado: apenas de formulacao_categorias_gmd, ou NULL
  ...
  FROM public.planos_nutricionais
  WHERE lote_id = p_lote_id AND ordem > v_plano.ordem AND data_inicio IS NULL AND data_fim IS NULL
  ORDER BY ordem ASC LIMIT 1;
  ...
  IF FOUND THEN
    ...
    LOOP
      -- GMD: apenas de formulacao_categorias_gmd, NULL se nao contemplada
      SELECT fcg.gmd INTO v_gmd_proximo
      FROM public.formulacao_categorias_gmd fcg
      WHERE fcg.formulacao_id = v_proximo_plano.formulacao_id
        AND LOWER(TRIM(fcg.categoria)) = LOWER(TRIM(v_cat.categoria));
      IF NOT FOUND THEN
        v_gmd_proximo := NULL;
      END IF;
