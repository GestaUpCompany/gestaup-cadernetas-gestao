-- Migration U: Fix dupla contagem com data_ajuste_peso + bezerros ao pe respeitam data_ajuste_peso + agendamento cron
--
-- Bug 1: branch data_ajuste_peso do primeiro LOOP somava 1 GMD por execucao, nao por dia.
--        Se o cron rodasse 2x no mesmo dia, dobrava o ganho.
--        Fix: calcular days_diff = CURRENT_DATE - data_ajuste_peso e somar gmd * days_diff.
--        Apos calcular, atualizar data_ajuste_peso = CURRENT_DATE para que a proxima execucao
--        no mesmo dia tenha days_diff = 0 e nao some nada.
--
-- Bug 2: segundo LOOP (bezerros ao pe) ignorava data_ajuste_peso e sempre recalculava
--        a partir de peso_entrada + (gmd * dias_desde_inicio), sobrescrevendo ajustes manuais.
--        Fix: se data_ajuste_peso IS NOT NULL, evoluir a partir do peso atual + (gmd * days_diff),
--        igual ao primeiro LOOP. Se data_ajuste_peso IS NULL, manter calculo original.
--
-- Bug 3: cron update_dados_lotes nao estava agendado em nenhuma migration.
--        Fix: agendar com cron.schedule('update-dados-lotes', '0 3 * * *', ...).
  ...
  LOOP
    ...
    ELSE
      ...
      IF categoria_record.data_ajuste_peso IS NOT NULL THEN
        ...
        END IF;
      ELSE
        -- Evolui a partir do peso de entrada/plano, somando GMD por dia desde o inicio
        peso_base := COALESCE(categoria_record.peso_inicio_kg_cab, categoria_record.peso_entrada_kg_cab);
...
-- Agendar cron (daily 03:00 UTC = 00:00 Brasilia)
-- Idempotente: so agenda se nao existir
DO $do$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM cron.job WHERE jobname = 'update-dados-lotes') THEN
    PERFORM cron.schedule('update-dados-lotes', '0 3 * * *', 'SELECT public.update_dados_lotes();');
  END IF;
END
$do$;
