  ...
  IF NEW.fazenda_id = 'd649c65e-16ab-4b77-a84b-df937aa41cc3' THEN
    ...
    IF NEW.lote_destino_id IS NOT NULL AND NEW.categoria IS NOT NULL THEN
      ...
      IF NOT EXISTS (
        ...
        -- Create new category with copied data (lote_id, quant_inicial, quant_atual are set separately)
        INSERT INTO lote_categorias (
          ...
          preco_animal_kg, preco_animal_cab, custo_operacional,
          morte, consumo, abate, transf_entrada, transf_saida, ativo
        ) VALUES (
          NEW.lote_destino_id, NEW.categoria, 0, 0,
          ...
          v_source_cat.preco_animal_cab, v_source_cat.custo_operacional,
          v_source_cat.morte, v_source_cat.consumo, v_source_cat.abate,
          v_source_cat.transf_entrada, v_source_cat.transf_saida, true
        );
      END IF;
