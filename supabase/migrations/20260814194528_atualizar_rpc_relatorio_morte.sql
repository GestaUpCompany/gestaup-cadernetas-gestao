CREATE OR REPLACE FUNCTION public.get_dados_relatorio_morte(
  ...
  p_preco_kg numeric DEFAULT 8.0
)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
DECLARE
  ...
  v_preco_kg_usado numeric;
  v_peso_total_perdido numeric;
  v_perda_estimada_reais numeric;
...
BEGIN
  ...
  v_preco_kg_usado := GREATEST(COALESCE(p_preco_kg, 8.0), 0.01);
  ...
  SELECT COALESCE(SUM(r.peso_vivo), 0)
  INTO v_peso_total_perdido
  FROM registros_morte r
  WHERE r.fazenda_id = v_fazenda_id
    AND r.deleted_at IS NULL
    AND r.peso_vivo IS NOT NULL
    AND (p_data_inicio IS NULL OR (r.data AT TIME ZONE v_timezone)::date >= p_data_inicio)
    AND (p_data_fim IS NULL OR (r.data AT TIME ZONE v_timezone)::date <= p_data_fim);

  v_peso_total_perdido := round(v_peso_total_perdido::numeric, 1);
  v_perda_estimada_reais := round((v_peso_total_perdido * v_preco_kg_usado)::numeric, 2);

  v_impacto_financeiro := jsonb_build_object(
    'perda_estimada_reais', v_perda_estimada_reais,
    'peso_total_perdido_kg', v_peso_total_perdido,
    'preco_kg_usado', v_preco_kg_usado
  );
...
GRANT EXECUTE ON FUNCTION public.get_dados_relatorio_morte(uuid, date, date, numeric) TO anon, authenticated;;
