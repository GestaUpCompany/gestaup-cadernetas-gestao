...
BEGIN
  -- Get quant_inicial from lote_categorias
  SELECT COALESCE(quant_inicial, 0) INTO v_quant_inicial
  FROM lote_categorias
  WHERE lote_id = p_lote_id AND categoria = p_categoria;

  -- Sum entradas from registros_movimentacao
  ...
  -- Sum saidas (consumo, abate, saída) from registros_movimentacao
  ...
  WHERE lote_destino_id = p_lote_id
    AND categoria = p_categoria
    AND tipo_entrada = 'Transferência'
    AND deleted_at IS NULL;
