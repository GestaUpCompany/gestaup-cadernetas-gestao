...
BEGIN
  FOR categoria_record IN 
    SELECT lc.id, lc.lote_id, lc.categoria, lc.peso_entrada, lc.gmd, lc.data_pesagem, lc.data_meta, lc.rc_inicial
    FROM lote_categorias lc
    WHERE lc.peso_entrada IS NOT NULL 
  ...
  LOOP
    ...
    IF categoria_record.rc_inicial IS NOT NULL THEN
      calc_peso_entrada_arrobas := (categoria_record.peso_entrada * (categoria_record.rc_inicial / 100)) / 15;
    ELSE
