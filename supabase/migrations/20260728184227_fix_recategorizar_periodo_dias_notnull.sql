...
BEGIN
  ...
  -- Caso A: origem tinha plano ativo -> criar novo plano copiando config do origem, com nova formulacao
  ...
  IF v_novo_formulacao_id IS NOT NULL THEN
    IF v_plano_origem IS NOT NULL THEN
      -- Caso A: copiar config do plano origem
      INSERT INTO public.planos_nutricionais (
        ...
        v_novo_lote_categoria_id, v_fazenda_id, v_plano_origem.nome, v_novo_formulacao_id,
        COALESCE(v_plano_origem.periodo_dias, 0), v_plano_origem.peso_meta_kg, 0, true, CURRENT_DATE,
        v_peso_transicao, v_plano_origem.rc_inicio, v_plano_origem.migracao_automatica, v_plano_origem.gmd_planejado,
    ...
    ELSIF NOT p_manter_formulacao THEN
      ...
      INSERT INTO public.planos_nutricionais (
        ...
        v_novo_lote_categoria_id, v_fazenda_id,
        p_categoria_destino || ' - ' || COALESCE(v_formulacao.gmd::text, '') || ' kg/dia',
        v_novo_formulacao_id,
        0, NULL, 0, true, CURRENT_DATE,
        v_peso_transicao, NULL, false, v_formulacao.gmd,
        'peso'
      );
    END IF;
