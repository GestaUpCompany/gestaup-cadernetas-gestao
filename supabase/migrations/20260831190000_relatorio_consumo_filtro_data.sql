-- Reescrita da RPC get_dados_relatorio_consumo para respeitar filtro de data em todos os campos
-- Antes: n_cabecas, peso, categoria, raca, dieta, dias vinham de lote_categorias/planos_nutricionais ativos HOJE
-- Agora: vêm do último registro de suplementação dentro do período filtrado + categorias/planos vigentes na data desse registro
...
BEGIN
  ...
  info_lotes AS (
    SELECT
      ...
      END AS dias,
      CASE WHEN cce.erros IS NULL THEN to_char(COALESCE(p_data_fim, (now() AT TIME ZONE 'America/Cuiaba')::date), 'YYYY-MM-DD') END AS data_prevista_final,
      cce.erros
