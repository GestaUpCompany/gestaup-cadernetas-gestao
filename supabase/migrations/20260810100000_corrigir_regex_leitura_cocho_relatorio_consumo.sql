-- Corrige regex de validação de leitura_cocho nas três RPCs de relatório de consumo.
--
-- Bug: as funções armazenadas no banco continham o regex '^[0-9]+\\.?[0-9]*$'
-- (com backslash duplicado). Com standard_conforming_strings = on, '\\.' na
-- string literal vira dois caracteres backslash no regex, que passa a exigir
-- um backslash literal após os dígitos. Nenhum valor numérico como "0" ou "1"
-- casa, então leitura_cocho voltava sempre NULL, ocultando o gráfico de
-- Leitura de Cocho no relatório público e no PDF.
--
-- Fix: substituir '\.' por character class '[.]', eliminando qualquer
-- dependência de escape de backslash. Novo regex: '^[0-9]+([.][0-9]*)?$'
-- (equivalente em semântica ao original pretendido '^[0-9]+\.?[0-9]*$').
--
-- Afeta:
--   1. get_dados_relatorio_consumo(p_token uuid, p_data_inicio date, p_data_fim date)
--   2. get_relatorio_consumo(p_token uuid, p_data_inicio date, p_data_fim date)
--   3. get_relatorio_consumo(p_token uuid, p_data_inicio date, p_data_fim date, p_fazenda_id_param uuid)

-- ============================================================
-- 1) get_dados_relatorio_consumo (versão pública via token)
-- ============================================================
...
-- ============================================================
-- 2) get_relatorio_consumo (versão interna via token, sem fazenda_id_param)
-- ============================================================
...
-- ============================================================
-- 3) get_relatorio_consumo (versão interna com p_fazenda_id_param)
-- ============================================================
CREATE OR REPLACE FUNCTION public.get_relatorio_consumo(
...
-- Permissões para anon e authenticated (mantém compatibilidade com a rota pública)
