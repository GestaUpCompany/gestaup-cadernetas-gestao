...
BEGIN
  ...
  -- 7. Encerrar plano nutricional ativo da origem (cria snapshot interno em planos_nutricionais_snapshots)
  IF FOUND THEN
  ...
  -- 10. Criar novo plano nutricional se houver formulacao
  IF v_novo_formulacao_id IS NOT NULL AND v_plano_origem IS NOT NULL THEN
...
GRANT EXECUTE ON FUNCTION public.recategorizar_lote_categoria(uuid, text, boolean, uuid, uuid, text) TO authenticated, service_role;
