-- pgRouting requer coluna id integer. mapa_estradas.id é uuid.
-- Adicionamos gid serial para usar como id de aresta no pgRouting.
ALTER TABLE public.mapa_estradas ADD COLUMN IF NOT EXISTS gid serial;
CREATE INDEX IF NOT EXISTS idx_mapa_estradas_gid ON public.mapa_estradas(gid);

-- Recriar encontrar_rota usando gid
...
BEGIN
  ...
  -- Limpar source/target antes de recriar topologia
  UPDATE public.mapa_estradas SET source = NULL, target = NULL WHERE ativo = true;

  -- pgr_createTopology requer id integer (gid), não uuid
  ...
  FROM pgr_dijkstra(
    'SELECT gid AS id, source, target, ST_Length(geometria::geography) AS cost, ST_Length(geometria::geography) AS reverse_cost FROM public.mapa_estradas WHERE ativo = true',
    v_start_vid,
    v_end_vid,
    false
  ) d
...
-- Recriar encontrar_rota_multi usando gid
