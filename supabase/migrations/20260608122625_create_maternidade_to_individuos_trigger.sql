-- Create function to create individual from maternidade record
...
BEGIN
  ...
  -- Lookup pasto UUID if pasto name is provided
  IF NEW.pasto IS NOT NULL THEN
...
-- Create trigger on registros_maternidade INSERT
CREATE TRIGGER trg_maternidade_create_individual
AFTER INSERT ON registros_maternidade
FOR EACH ROW
EXECUTE FUNCTION create_individual_from_maternidade();;
