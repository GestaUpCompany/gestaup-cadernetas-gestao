  ...
  IF NEW.fazenda_id = 'd649c65e-16ab-4b77-a84b-df937aa41cc3' THEN
    ...
    IF NEW.lote_destino_id IS NOT NULL AND NEW.categoria IS NOT NULL THEN
      ...
      IF NOT EXISTS (
        ...
        -- Calculate quant_atual for the new category
        v_quant_atual := calculate_quant_atual(NEW.lote_destino_id, NEW.categoria);
        
        -- Create new category with copied data (excluding morte, consumo, abate, transf_entrada, transf_saida)
        INSERT INTO lote_categorias (
          ...
          NEW.lote_destino_id, NEW.categoria, v_quant_atual, v_quant_atual,
