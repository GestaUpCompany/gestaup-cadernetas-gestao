...
BEGIN
  ...
  FOR v_cat_item IN SELECT * FROM jsonb_array_elements(p_categorias) LOOP
    ...
    IF v_cabecas IS NULL OR v_cabecas <= 0 THEN
      RETURN jsonb_build_object('success', false, 'error', 'Cabeças invalidas para categoria: ' || v_categoria);
    END IF;
    ...
    IF v_cabecas > COALESCE(v_cat_origem.quant_atual, 0) THEN
      RETURN jsonb_build_object('success', false, 'error', 'Cabeças excedem disponivel para categoria: ' || v_categoria);
    END IF;
  ...
  SELECT string_agg(c->>'categoria' || ': ' || (c->>'numero_cabecas') || ' cabeças', ', ')
  ...
  LOOP
    ...
    VALUES (
      ...
      'info',
      'Lote recebido por transferência: ' || v_novo_lote_nome,
      'Lote "' || v_lote.nome || '" recebido da fazenda ' || v_fazenda_origem.nome || ' por transferência. Categorias: ' || v_categorias_desc || '. Total: ' || v_total_transferir || ' cabeças.',
      '/controller/lotes',
      'Ver lotes',
      jsonb_build_object(
  ...
  LOOP
    ...
    VALUES (
      ...
      'Lote "' || v_lote.nome || '" transferido para a fazenda ' || v_fazenda_destino.nome || '. Categorias: ' || v_categorias_desc || '. Total: ' || v_total_transferir || ' cabeças.' || CASE WHEN v_is_total THEN ' Lote inativado na origem.' ELSE ' Lote permanece ativo com ' || v_total_restante || ' cabeças.' END,
      '/controller/lotes',
      'Ver lotes',
      jsonb_build_object(
