...
-- ============================================================================
-- Função: migrar_plano_nutricional
-- - p_lote_categoria_id: id da categoria no lote
-- - p_plano_destino_id: id do plano destino (NULL = próximo em ordem)
-- - p_motivo: 'periodo_completo' | 'peso_atingido' | 'ambos' | 'manual'
-- ============================================================================
...
BEGIN
  ...
  IF NOT FOUND THEN
    -- Se não há plano vigente, buscar o de menor ordem para tornar vigente
    SELECT * INTO v_plano_atual
    FROM public.planos_nutricionais
    WHERE lote_categoria_id = p_lote_categoria_id
    ORDER BY ordem ASC
  ...
  IF p_plano_destino_id IS NOT NULL THEN
    ...
    END IF;
  ELSE
    -- Próximo plano em ordem que ainda não foi executado
    SELECT * INTO v_proximo_plano
    FROM public.planos_nutricionais
    WHERE lote_categoria_id = p_lote_categoria_id
  ...
  -- Não pode migrar para o mesmo plano
  ...
  -- Custo operacional total e outras métricas em JSONB
  ...
  -- Atualizar lote_categorias com dados do novo plano (GMD vem da formulação)
  ...
  -- Propagar para individuos acontece automaticamente via trigger
END;
$function$;
;
