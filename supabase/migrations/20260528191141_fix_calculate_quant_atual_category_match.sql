...
BEGIN
  ...
  -- Sum saidas (consumo, abate, saída) from registros_movimentacao (case-insensitive)
  ...
  -- Sum transferências entrada (destination) - includes Apartação and Transferência, OR if lote_destino_id is set (treat as entrada)
  -- For destination, we don't filter by categoria since animals may change categories when moving
