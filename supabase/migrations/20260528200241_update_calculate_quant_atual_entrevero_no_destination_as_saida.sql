...
BEGIN
  ...
  -- Sum saidas (consumo, abate, saída, entrevero without destination) from registros_movimentacao - EXCLUDE transfers
  SELECT COALESCE(SUM(numero_cabecas), 0) INTO v_sum_saidas
  FROM registros_movimentacao
  WHERE lote_origem_id = p_lote_id
    AND LOWER(categoria) = LOWER(p_categoria)
    AND (motivo_movimentacao IN ('Consumo', 'Abate', 'Saída') OR (motivo_movimentacao = 'Entrevero' AND lote_destino_id IS NULL))
    AND tipo_saida IS NULL
    AND deleted_at IS NULL;

  -- Sum transferências saida (source) - includes Apartação, Transferência, and Entrevero (with destination) - case-insensitive
