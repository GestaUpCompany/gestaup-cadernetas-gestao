...
BEGIN
  ...
  WITH registros_windowed AS (
    ...
    SELECT lcr.lote_id, l.nome AS lote_nome,
      lc.peso_entrada_kg_cab, lc.peso_vivo_atual_kg_cab,
      to_char(lc.data_meta_projetada, 'YYYY-MM-DD') AS data_prevista_final,
      lc.quant_atual AS n_cabecas_atual, lc.raca, lc.categoria,
      f.nome AS dieta, to_char(pn.data_inicio, 'YYYY-MM-DD') AS data_inicio_plano,
  ...
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
    ...
    'info', jsonb_build_object(
      ...
      'raca', il.raca, 'categoria', il.categoria,
      'dieta', il.dieta, 'data_inicio_plano', il.data_inicio_plano,
      'dias', il.dias
    ),
