-- Corrige RPC get_dados_relatorio_tratos para filtrar apenas lotes ativos no slicer
...
BEGIN
  ...
  -- Lotes disponíveis (apenas ativos com registros, sem filtro de data, para o slicer)
  ...
  SELECT COALESCE(jsonb_agg(jsonb_build_object('data', data, 'lote_id', lote_id, 'lote_nome', lote_nome, 'curral_nome', curral_nome, 'ordem_trato', ordem_trato, 'kg_planejado', kg_planejado, 'kg_ofertado_real', kg_ofertado_real, 'desvio_kg', desvio_kg, 'desvio_pct', desvio_pct, 'leitura_cocho_nota', leitura_cocho_nota, 'horario_sugerido', horario_sugerido, 'horario_real', horario_real, 'desvio_min', desvio_min, 'tratador', tratador, 'status_kg', status_kg, 'status_horario', status_horario) ORDER BY data DESC, lote_nome, ordem_trato), '[]'::jsonb)
  INTO v_linhas FROM linhas_calc;

  WITH resumo_calc AS (
    SELECT lote_id, lote_nome, COALESCE(SUM(kg_planejado), 0) AS planejado_total_kg, COALESCE(SUM(kg_ofertado_real), 0) AS real_total_kg, COALESCE(SUM(desvio_kg), 0) AS desvio_total_kg, CASE WHEN SUM(kg_planejado) > 0 THEN round((SUM(desvio_kg) / SUM(kg_planejado) * 100)::numeric, 2) ELSE NULL END AS desvio_medio_pct, COUNT(DISTINCT data) AS dias_com_registro, COUNT(*) AS n_tratos, COUNT(*) FILTER (WHERE status_kg = 'ok') AS n_ok, COUNT(*) FILTER (WHERE status_kg = 'alerta') AS n_alerta, COUNT(*) FILTER (WHERE status_kg = 'critico') AS n_critico, COUNT(*) FILTER (WHERE status_kg = 'sem_execucao') AS n_sem_execucao, COUNT(*) FILTER (WHERE status_horario = 'ok') AS n_horario_ok, COUNT(*) FILTER (WHERE status_horario = 'alerta') AS n_horario_alerta, COUNT(*) FILTER (WHERE status_horario = 'critico') AS n_horario_critico, CASE WHEN COUNT(*) FILTER (WHERE desvio_min IS NOT NULL) > 0 THEN round(avg(desvio_min) FILTER (WHERE desvio_min IS NOT NULL)::numeric, 1) ELSE NULL END AS desvio_medio_min, CASE WHEN SUM(kg_planejado) > 0 THEN CASE WHEN abs(SUM(desvio_kg) / SUM(kg_planejado) * 100) <= 5 THEN 'ok' WHEN abs(SUM(desvio_kg) / SUM(kg_planejado) * 100) <= 15 THEN 'alerta' ELSE 'critico' END WHEN COALESCE(SUM(kg_ofertado_real), 0) = 0 THEN 'sem_execucao' ELSE 'ok' END AS status
    FROM (SELECT r.data_dia AS data, r.lote_id, r.lote_nome, r.kg_planejado, r.kg_ofertado_real, CASE WHEN r.kg_ofertado_real IS NULL OR r.kg_ofertado_real = 0 THEN NULL ELSE r.kg_ofertado_real - r.kg_planejado END AS desvio_kg, r.horario_sugerido_str, r.horario_real, CASE WHEN r.horario_sugerido_str IS NULL OR r.horario_real IS NULL THEN NULL ELSE (EXTRACT(HOUR FROM r.horario_real::time) * 60 + EXTRACT(MINUTE FROM r.horario_real::time) - EXTRACT(HOUR FROM r.horario_sugerido) * 60 - EXTRACT(MINUTE FROM r.horario_sugerido)) END AS desvio_min, CASE WHEN r.kg_ofertado_real IS NULL OR r.kg_ofertado_real = 0 THEN 'sem_execucao' WHEN r.kg_planejado > 0 AND abs((r.kg_ofertado_real - r.kg_planejado) / r.kg_planejado * 100) <= 5 THEN 'ok' WHEN r.kg_planejado > 0 AND abs((r.kg_ofertado_real - r.kg_planejado) / r.kg_planejado * 100) <= 15 THEN 'alerta' ELSE 'critico' END AS status_kg, CASE WHEN r.horario_sugerido_str IS NULL THEN 'sem_horario' WHEN abs(EXTRACT(HOUR FROM r.horario_real::time) * 60 + EXTRACT(MINUTE FROM r.horario_real::time) - EXTRACT(HOUR FROM r.horario_sugerido) * 60 - EXTRACT(MINUTE FROM r.horario_sugerido)) <= 15 THEN 'ok' WHEN abs(EXTRACT(HOUR FROM r.horario_real::time) * 60 + EXTRACT(MINUTE FROM r.horario_real::time) - EXTRACT(HOUR FROM r.horario_sugerido) * 60 - EXTRACT(MINUTE FROM r.horario_sugerido)) <= 30 THEN 'alerta' ELSE 'critico' END AS status_horario FROM regs_com_sugerido r) calc
    GROUP BY lote_id, lote_nome
  )
  SELECT COALESCE(jsonb_agg(jsonb_build_object('lote_id', lote_id, 'lote_nome', lote_nome, 'planejado_total_kg', planejado_total_kg, 'real_total_kg', real_total_kg, 'desvio_total_kg', desvio_total_kg, 'desvio_medio_pct', desvio_medio_pct, 'dias_com_registro', dias_com_registro, 'n_tratos', n_tratos, 'n_ok', n_ok, 'n_alerta', n_alerta, 'n_critico', n_critico, 'n_sem_execucao', n_sem_execucao, 'n_horario_ok', n_horario_ok, 'n_horario_alerta', n_horario_alerta, 'n_horario_critico', n_horario_critico, 'desvio_medio_min', desvio_medio_min, 'status', status) ORDER BY lote_nome), '[]'::jsonb)
  INTO v_resumo FROM resumo_calc;

  RETURN jsonb_build_object('fazenda_nome', v_fazenda_nome, 'fazenda_logo_url', v_fazenda_logo_url, 'timezone', v_timezone, 'lotes_disponiveis', v_lotes_disponiveis, 'linhas', v_linhas, 'resumo_por_lote', v_resumo);
END;
$function$;;
