-- F1.2: Atualizar RPC gerar_notificacoes_recategorizacao para ler config da fazenda
...
DECLARE
  ...
  v_mensagem text;
  v_gmd numeric;
  v_threshold numeric := 95.0; -- default
  v_ativo boolean := true; -- default
  v_config RECORD;
BEGIN
  ...
  -- Se recategorizacao_ativo = false, nao processar
  IF NOT v_ativo THEN
    RETURN 0;
  END IF;

  -- Iterar sobre lote_categorias ativas que atingiram o threshold da faixa
  FOR v_lote IN
    SELECT 
...
SELECT 'RPC gerar_notificacoes_recategorizacao atualizada para ler config' AS status;;
