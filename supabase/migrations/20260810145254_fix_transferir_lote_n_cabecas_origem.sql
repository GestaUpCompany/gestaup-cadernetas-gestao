...
BEGIN
  ...
  ELSE
    FOR v_cat_item IN SELECT * FROM jsonb_array_elements(p_categorias) LOOP
      ...
      WHERE lote_id = p_lote_origem_id
        ...
        AND ativo = true;
    END LOOP;
    -- Usar soma real das categorias (fonte de verdade) em vez do campo n_cabecas que pode estar desatualizado
    UPDATE lotes
  ...
  LOOP
    ...
    VALUES (
      ...
      'Lote "' || v_lote.nome || '" transferido para a fazenda ' || v_fazenda_destino.nome || '. Categorias: ' || v_categorias_desc || '. Total: ' || v_total_transferir || ' cabecas.' || CASE WHEN v_is_total THEN ' Lote inativado na origem.' ELSE ' Lote permanece ativo com ' || v_total_restante || ' cabecas.' END,
      '/controller/lotes',
      'Ver lotes',
      jsonb_build_object(
