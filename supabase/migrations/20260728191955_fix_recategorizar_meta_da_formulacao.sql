...
BEGIN
  ...
  -- Caso A: origem tinha plano ativo -> criar novo plano com GMD e meta da nova formulação (se trocou) ou do origem (se manteve)
  ...
  IF v_novo_formulacao_id IS NOT NULL THEN
    -- Buscar GMD e peso_vivo_medio da nova formulação
    ...
    -- Determinar peso_meta: se trocou formulação, usar peso_vivo_medio da nova; se manteve, preservar meta do origem
    ...
    IF v_tem_plano_origem THEN
      -- Caso A: criar plano com GMD e meta da nova formulação (se trocou) ou do origem (se manteve)
