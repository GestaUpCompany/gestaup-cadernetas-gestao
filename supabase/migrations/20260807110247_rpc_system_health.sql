  ...
  SELECT jsonb_build_object(
    ...
    'fazendas', jsonb_build_object(
      ...
      'withUsers24h', (SELECT COUNT(DISTINCT f.id)
        ...
        JOIN public.fazenda_usuarios fu ON fu.fazenda_id = f.id
        JOIN auth.users u ON u.id = fu.usuario_id
    ...
    'topTables', COALESCE((
      SELECT jsonb_agg(jsonb_build_object(
        ...
        'seqScans', n_tup_ins + COALESCE(seq_scan, 0)
      ))
      FROM (
        SELECT relname, relid, n_tup_ins, seq_scan
