-- Corrigir acentos faltantes na notificacao de morte
-- "Pasto nao informado" -> "Pasto não informado"
-- "Causa nao informada" -> "Causa não informada"
...
BEGIN
    ...
    v_causa := COALESCE(NEW.causa_morte, 'Causa não informada');

    -- Pasto (pode ser null)
    v_pasto := COALESCE(NEW.pasto, 'Pasto não informado');
