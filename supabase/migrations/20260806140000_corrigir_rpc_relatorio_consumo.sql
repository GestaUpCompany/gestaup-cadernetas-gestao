-- Corrige bug na RPC get_dados_relatorio_consumo que causava erro 400
-- ao abrir link público de relatório de consumo.
--
-- Problema: a query de v_lotes_disponiveis tinha ORDER BY l.nome no nível
-- externo, mas jsonb_agg sem GROUP BY produz uma única linha, e PostgreSQL
-- não permite ORDER BY referenciando colunas não agregadas nesse contexto.
-- O ORDER BY dentro de jsonb_agg(... ORDER BY l.nome) já garante a ordenação.
--
-- Sintoma: Failed to load resource: 400 em
-- /rest/v1/rpc/get_dados_relatorio_consumo ao abrir relatório público.
