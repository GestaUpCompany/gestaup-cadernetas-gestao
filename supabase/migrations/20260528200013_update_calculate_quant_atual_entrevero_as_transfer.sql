...
BEGIN
  ...
  -- Sum transferências saida (source) - includes Apartação, Transferência, and Entrevero - case-insensitive
  SELECT COALESCE(SUM(numero_cabecas), 0) INTO v_sum_transf_saida
  FROM registros_movimentacao
  WHERE lote_origem_id = p_lote_id
    AND LOWER(categoria) = LOWER(p_categoria)
    AND (tipo_saida IN ('Transferência', 'Apartação') OR motivo_movimentacao = 'Entrevero')
    AND deleted_at IS NULL;
